...
class HardConcreteGate(nn.Module):
    """
    Per-band stochastic on/off gates with L0 regularization.
    Paper: Louizos et al., "Learning Sparse Neural Networks through L_0 Regularization".
    z ~ HardConcrete(p); expected L0 = P(z>0) = sigmoid(log_alpha - beta*log(-gamma/zeta))
    """
    def __init__(self, C=25, init_p=0.5, beta=2./3., gamma=-0.1, zeta=1.1):
        super().__init__()
        # parameterize p via logit
        self.log_alpha = nn.Parameter(torch.logit(torch.full((C,), float(init_p))))
        self.beta = beta
        self.gamma = gamma
        self.zeta = zeta

    def expected_L0(self):
        # Prob gate is nonzero under hard-concrete
    ...
    def forward(self, x):
        if self.training:
            ...
            # use expected gate at eval
            s = torch.sigmoid(self.log_alpha)
            z = (s * (self.zeta - self.gamma) + self.gamma).clamp(0, 1)
        return x * z.view(1, -1, 1, 1), z, self.expected_L0()
...
class YOLOLite(nn.Module):
    ...
    def __init__(self, nc=2, img_size=(DEMOSAIC_H, DEMOSAIC_W), anchors=((10,12),(16,19),(23,33))):
        ...
        self.gate = HardConcreteGate(25, init_p=0.5)  # start neutral(ish)
    ...
    def forward(self, x):
        x, z_sample, p_expect = self.gate(x)   # z_sample=[25], p_expect=[25]
...
# ----------------------- Logging -----------------------
def log_band_importances(p_vec, epoch, top_k=10):
    vals = p_vec.detach().cpu().tolist()  # probabilities p(z>0)
    ranked = sorted(list(zip(BAND_ORDER, vals)), key=lambda kv: kv[1], reverse=True)
    print(f"\n[Epoch {epoch}] Band importances (p(z>0)):")
    for i, (code, v) in enumerate(ranked):
        tag = "★" if i < top_k else " "
        print(f"  {tag} band_code={code:>3}  p={v:.4f}")
    print()
...
def init_csv(csv_path: Path, band_order: list[int]):
    ...
    if is_new:
        headers = ["epoch", "train_loss", "val_loss"] + [f"p_{b}" for b in band_order]
        with open(csv_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(headers)

def append_row(csv_path: Path, epoch: int, train_loss: float, val_loss: float, p_vec: list[float], band_order: list[int]):
    row = [epoch, train_loss, val_loss] + [float(a) for a in p_vec]
...
def plot_band_probs(out_path: Path, epochs: list[int], p_hist: list[list[float]], band_order: list[int]):
    """
    One plot with all 25 band keep probabilities over epochs + legend (key).
    Colors are stable across bands using a discrete colormap.
    """
    A = np.array(p_hist, dtype=np.float32)  # [num_epochs, 25]
    ...
    plt.xlabel("epoch")
    plt.ylabel("p(z>0)")
    plt.title("Band keep probabilities over epochs")
...
# ----------------------- Training -----------------------
def train(data_yaml: Path, epochs=50, batch=16, lr=2e-3, l0=1e-3, workers=4, logdir: Path=Path("runs/bandgate")):
    # Parse data.yaml
    ...
    gate_params = [p for n,p in model.named_parameters() if n.startswith("gate.")]
    ...
    for epoch in range(1, epochs+1):
        ...
        with torch.no_grad():
            pvec = model.gate.expected_L0().detach().cpu()
            print(f"[Epoch {epoch}] gate p(z>0) range: {pvec.min():.3f}..{pvec.max():.3f}")
        ...
        for cubes, targets, stems in train_loader:
            ...
            pred, z_sample, p_expect = model(cubes)
            total, parts = loss_fn(pred, targets)

            # L0 penalty on expected number of active bands
            l0_pen = p_expect.sum()
            loss = total + l0 * l0_pen
        ...
        with torch.no_grad():
            ...
            # use model's current expected keep prob (data-independent, stable)
            a_now = model.gate.expected_L0().detach().cpu().numpy().tolist()
            ...
            plot_band_probs(alphas_png, epochs_hist, p_hist, BAND_ORDER)

            # (console peek remains handy)
            print(f"[Epoch {epoch}] gate p(z>0) range: {min(a_now):.3f}..{max(a_now):.3f}")
            log_band_importances(model.gate.expected_L0(), epoch, top_k=10)

        print(f"Epoch {epoch:03d} | train_loss {running/len(train_loader):.4f} | val_loss {val_loss:.4f}")

    p_vec = model.gate.expected_L0().detach().cpu().numpy().tolist()
    torch.save({
        "state_dict": model.state_dict(),
        "gate_expected_p": p_vec,   # p(z>0) per band
        "band_order": BAND_ORDER,
        "names": names
    }, "bandgated_detector.pt")
    print("\nSaved → bandgated_detector.pt")
    print("Final band importances (band_code: p(z>0)):")
    for code, p in sorted(zip(BAND_ORDER, p_vec), key=lambda kv: kv[1], reverse=True):
        print(f"  {code}: {p:.4f}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--l0", type=float, default=5e-3, help="L0 sparsity penalty on expected gates (sum p_i)")

    args = ap.parse_args()
    train(args.data, epochs=args.epochs, batch=args.batch, lr=args.lr, l0=args.l0, workers=args.workers, logdir=args.logdir)
